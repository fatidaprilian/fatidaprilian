<img src="https://capsule-render.vercel.app/api?type=waving&color=0:211A1C,45:E8A989,100:F8DCC8&height=200&section=header&text=Farid%20Eka%20Aprilian%20%E2%9C%A8&fontSize=46&fontColor=FFF8F2&animation=twinkling&fontAlignY=32&desc=Web%20Systems%20%7C%20APIs%20%7C%20Product%20Engineering&descSize=16&descAlignY=55" width="100%" alt="Farid Eka Aprilian" />

<div>

<img width="300" src="./img/fatidaprilian-profile.png" align="right" alt="Profile artwork" />

<br/>

<img src="./img/fatidaprilian-about.png" width="488" alt="About me" />

<br/>
<br/>

- Name: **Farid Eka Aprilian**.

- Location: **Indonesia**.

- Focus: **Web systems**, **API architecture**, and **product-oriented development**.

- Backend: **NestJS**, **Node.js**, **Laravel**, **Go**, and service integration.

- Frontend: **Next.js**, **React**, **Vue**, and **Tailwind CSS**.

- Data and Infra: **PostgreSQL**, **MySQL**, **Redis**, **Docker**, and **GitHub Actions**.

- Languages: **TypeScript**, **JavaScript**, **Go**, and **PHP**.

<br/>

<img src="./img/fatidaprilian-project.png" width="488" alt="Featured repositories" />

<br/>
<br/>

- [***fatidaprilian/ecommercevespa-client***](https://github.com/fatidaprilian/ecommercevespa-client) <br/>
  E-commerce client-side application built with TypeScript.

- [***fatidaprilian/digital-credential-platform***](https://github.com/fatidaprilian/digital-credential-platform) <br/>
  Credential management platform with a backend-focused TypeScript architecture.

- [***fatidaprilian/project-management-app-nodejs***](https://github.com/fatidaprilian/project-management-app-nodejs) <br/>
  Project management application using a Node.js and TypeScript stack.

<br/>

<img src="./img/fatidaprilian-decor.png" width="270" align="right" alt="Pointer artwork" />

<br/>

<img src="./img/fatidaprilian-banner.png" width="500" alt="Personal banner" /><br/>

<sub>Building useful systems with clean APIs, reliable delivery, and just enough polish to make the work feel alive.</sub>

<br/>
<br/>

<p align="center">
  <a href="#"><img src="https://img.shields.io/badge/%23-0F766E?style=for-the-badge" alt="#" /></a>
  <a href="mailto:faridaprilian215@gmail.com"><img src="https://img.shields.io/badge/EMAIL-0F766E?style=for-the-badge&logo=gmail&logoColor=white" alt="Email" /></a>
  <a href="https://github.com/fatidaprilian?tab=repositories"><img src="https://img.shields.io/badge/REPOSITORIES-0F766E?style=for-the-badge&logo=github&logoColor=white" alt="Repositories" /></a>
  <a href="https://faridekaaprilian.vercel.app/"><img src="https://img.shields.io/badge/WEBSITE-0F766E?style=for-the-badge" alt="Website" /></a>
  <a href="#"><img src="https://img.shields.io/badge/LINKEDIN-0F766E?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" /></a>
  <a href="#"><img src="https://img.shields.io/badge/%23-0F766E?style=for-the-badge" alt="#" /></a>
</p>

## GitHub Stats

<p align="center">
  <img height="170" src="https://github-readme-stats-farid.vercel.app/api?username=fatidaprilian&show_icons=true&hide_border=true&title_color=0f766e&icon_color=0f766e&text_color=334155&bg_color=ffffff" alt="GitHub Stats" />
  <img height="170" src="https://github-readme-stats-farid.vercel.app/api/top-langs/?username=fatidaprilian&layout=compact&langs_count=8&hide_border=true&title_color=0f766e&text_color=334155&bg_color=ffffff" alt="Top Languages" />
</p>

<p align="center">
  <img src="https://cdn.jsdelivr.net/gh/fatidaprilian/fatidaprilian@output/github-contribution-grid-snake.svg" alt="Contribution Snake" />
</p>

</div>
